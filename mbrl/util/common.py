...
def rollout_agent_trajectories_vector(
    env: gym.Env,
    steps_or_trials_to_collect: int,
    agent: mbrl.planning.Agent,
    agent_kwargs: Dict,
    ...
    while True:
        ...
        while True:
            if replay_buffer is not None:
                next_obs, reward, terminated, truncated, _ = step_env_and_add_to_buffer_batch(
                    env,
                    obs,
                    agent,
                    agent_kwargs,
                ...
                if callback:
                    callback((obs, action, next_obs, reward, terminated, truncated))
            obs = next_obs
            total_reward += reward #TODO: adapt to vector implementation
            step += env.num_envs #TODO: adapt to vector
            if not collect_full_trajectories and step == steps_or_trials_to_collect:
                total_rewards.append(total_reward)
                return total_rewards
            if trial_length and step % trial_length == 0:
...
def step_env_and_add_to_buffer(
    ...
    if not terminated:
        ...
        replay_buffer.add(obs, action, obs, reward, terminated, truncated)
    if callback:
        callback((obs, action, next_obs, reward, terminated, truncated))
    return next_obs, reward, terminated, truncated, info

def step_env_and_add_to_buffer_batch(
    env: gym.Env,
    obs: np.ndarray,
    agent: mbrl.planning.Agent,
    agent_kwargs: Dict,
    ...
    next_obs, reward, terminated, truncated, info = env.step(action)
    next_obs_modified = next_obs.copy()
    terminated_envs_index = np.where(terminated)
    # print(next_obs_modified[terminated_envs_index])
    # Replace observation
    next_obs_modified[terminated_envs_index] = agent_obs[terminated_envs_index]
    # Delete observations
    # np.delete(obs, terminated_envs_index, axis=0)
    # np.delete(next_obs_modified, terminated_envs_index, axis=0)
    # np.delete(reward, terminated_envs_index, axis=0)
    # np.delete(terminated, terminated_envs_index, axis=0)
    # np.delete(truncated, terminated_envs_index, axis=0)
    replay_buffer.add_batch(obs, action, next_obs_modified, reward, terminated, truncated)
    if callback:
        callback((obs, action, next_obs, reward, terminated, truncated))
    return next_obs, reward, terminated, truncated, info